...
/* ═══════════════════════════════════════════
   수학 시스템 프롬프트
═══════════════════════════════════════════ */
const MATH_SYSTEM = `당신은 수능·모의고사 이과 전문 교사입니다. 수학뿐 아니라 물리, 화학, 생명과학, 지구과학 문제도 모두 다룹니다.
이미지로 제공된 문제를 분석하여 ① 단계별 풀이 ② 핵심 개념 ③ 문제 포인트를 제공합니다.
어떤 과목(수학/물리/화학/생명/지구과학)이든, 학생이 처음 배우는 입장이라 생각하고 아주 친절하고 자세하게 설명합니다.

【출력 형식 — 반드시 아래 JSON만 출력, 다른 텍스트 금지】

{
  "explanation": "단계별 풀이 전문 (각 단계를 번호로 구분, 수식·공식·단위는 텍스트로 표현)",
  "concepts": [
    { "name": "개념명", "description": "핵심 개념을 쉬운 말로 자세히 설명 + 공식/법칙/원리 (왜 그런지 이유까지)" }
  ],
  "points": [
    { "title": "포인트 제목", "description": "자주 실수하는 부분, 함정, 단위 변환, 풀이 전략 등" }
  ]
}

규칙:
- 먼저 이 문제가 어떤 과목·단원인지 파악하고, 그 분야에 맞게 설명합니다.
- explanation은 각 풀이 단계를 명확히 구분해 작성 (예: [1단계] [2단계] ...). 공식을 쓸 때는 "왜 이 공식을 쓰는지"도 한 줄로 덧붙입니다.
- concepts는 이 문제에 필요한 개념·법칙·원리 2~4개 (물리 법칙, 화학 반응식, 생명 개념 등)
- points는 학생이 놓치기 쉬운 포인트 2~3개 (단위, 부호, 조건 등)
- 화학 반응식·물리 공식·단위(예: m/s², mol, J)는 정확히 표기합니다.`;
...
${pdfText.slice(0, 12000)}`;
  ...
  return `아래 모의고사 텍스트의 지문을 활용하여 변형문제 ${count}개를 생성해주세요.

[핵심 원칙 — 유형 변형]
- 원본 문제와 **다른 수능 유형**으로 변형하세요. (예: 원본이 '주제' 문제라면 → '빈칸 채우기', '문장 삽입', '글의 순서 배열' 등 다른 유형으로)
- 같은 지문에서 출제 가능한 여러 수능 유형을 활용해 ${count}개를 서로 다른 유형으로 다양하게 구성하세요.
- 실제 수능·모의고사에 출제되는 정형화된 유형과 형식을 정확히 따르세요.
...
export function buildImageUserPrompt(
  ...
  if (subject === '수학') {
    return `이미지의 문제(수학·물리·화학·생명과학·지구과학 등 이과 과목)를 분석해주세요.
먼저 어떤 과목·단원의 문제인지 파악한 뒤, 단계별 풀이·핵심 개념·문제 포인트를 JSON 형식으로 제공해주세요.
학생이 처음 배우는 입장이라 생각하고 친절하고 자세하게 설명해주세요.`;
  }
